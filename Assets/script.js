...
function initialize() {}
savedLocations = JSON.parse(localStorage.getItem("wethercities"));
var lastSearch;
if (savedLocations){
    currentLoc = savedLocations[savedLocations.lenght -1];
    showPrevious (currentLoc);

}
else{
    if(!navigator.geolocation) {
        getCurrent("sao jose do rio preto")
    }
}
...
//                   var currentUV = currentTemp.append("<p>" + "UV Index: " + response.value + "</p>").addClass("card-text").setAttribute("class","badge");
...
//                   "<p>" + "Temperature: " + response.list[i].main.temp + "</p>" + "<p>" + "Humidity: " + response.list[i].main.humidity + 
//                   "%" + "</p>" + "</div>");
